...
/**
 * GET /api/auth/oauth/okta/callback
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        new URL(
          `/login?error=${encodeURIComponent('Okta sign-in failed')}`,
          request.url
        )
      );
    }
    ...
    const auth = getAuth();
    const result = await auth.exchangeOAuthCode('okta', {
      code,
    ...
    return NextResponse.redirect(
      new URL(
        `/login?error=${encodeURIComponent(
          error instanceof Error ? error.message : 'Failed to sign in with Okta'
        )}`,
        request.url
      )
    );
