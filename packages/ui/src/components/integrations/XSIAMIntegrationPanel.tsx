...
/**
 * XSIAM Integration Panel Component
 * Migrated from henryreed.ai/hosting/components/XSIAMIntegrationPanel.tsx
 *
 * Comprehensive XSIAM tenant integration interface with:
 * - Secure credential management
 * - Connection testing and status monitoring
 * - Health monitoring dashboard
 * - Analytics and threat intelligence
 * - Custom XQL query execution
 * - Real-time data refresh
 * - Multi-tab interface (Setup, Health, Analytics, Query)
 */

import React, { useState, useEffect } from 'react';
import {
  xsiamApiService,
  type XSIAMCredentials,
  type XSIAMHealthData,
  type XSIAMAnalyticsData
...
export interface XSIAMIntegrationPanelProps {
  /** Initial credentials (if any) */
  initialCredentials?: Partial<XSIAMCredentials>;
  /** Callback when connection status changes */
  onConnectionChange?: (status: ConnectionStatus) => void;
  /** Callback when credentials are saved */
  onCredentialsSaved?: (credentials: XSIAMCredentials) => void;
}

export const XSIAMIntegrationPanel: React.FC<XSIAMIntegrationPanelProps> = ({
  initialCredentials,
  onConnectionChange,
  onCredentialsSaved
}) => {
  const [credentials, setCredentials] = useState<XSIAMCredentials>({
    apiAddress: initialCredentials?.apiAddress || '',
    apiId: initialCredentials?.apiId || '',
    apiKey: initialCredentials?.apiKey || '',
    tenantName: initialCredentials?.tenantName || '',
  ...
  const handleInputChange = (field: keyof XSIAMCredentials, value: string) => {
    setCredentials((prev: XSIAMCredentials) => ({ ...prev, [field]: value }));
  };

  const handleConnect = async () => {
    setLoading(true);
    const connectingStatus: ConnectionStatus = { status: 'connecting', message: 'Testing connection...' };
    setConnectionStatus(connectingStatus);
    onConnectionChange?.(connectingStatus);
    ...
    try {
      ...
      if (isValid) {
        ...
        onConnectionChange?.(connectedStatus);
  ...
  const handleDisconnect = () => {
    ...
    onConnectionChange?.(disconnectedStatus);
    setHealthData(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex space-x-1 bg-gray-800 rounded-lg p-1">
        {[
          { id: 'setup' as const, name: 'Setup', icon: '⚙️', description: 'Configure connection' },
          { id: 'health' as const, name: 'Health', icon: '💓', disabled: connectionStatus.status !== 'connected', description: 'System status' },
          { id: 'analytics' as const, name: 'Analytics', icon: '📊', disabled: connectionStatus.status !== 'connected', description: 'Security metrics' },
          { id: 'query' as const, name: 'Query', icon: '🔍', disabled: connectionStatus.status !== 'connected', description: 'Custom XQL queries' }
        ].map(tab => (
          <button
            key={tab.id}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-gray-800 rounded-lg p-6">
                  ...
                  <div className="space-y-3">
                    {healthData.components.map((component: any, index: number) => (
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-gray-800 rounded-lg p-6">
                  ...
                  <div className="space-y-3">
                    {analyticsData.detectionCoverage.map((detection: any, index: number) => (
...
export default XSIAMIntegrationPanel;
