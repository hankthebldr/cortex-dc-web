...
/**
 * TRR Progress Chart Component
 * Migrated from henryreed.ai/hosting/components/TRRProgressChart.tsx
 *
 * Features:
 * - Multiple chart types (completion, velocity, burndown, risk, workload)
 * - Flexible grouping and timeframes
...
export interface TRR {
  id: string;
  title: string;
  status: 'draft' | 'pending' | 'in-progress' | 'validated' | 'completed';
...
export interface TRRProgressChartProps {
  trrs: TRR[];
...
export const TRRProgressChart: React.FC<TRRProgressChartProps> = ({
  ...
  function calculateCompletionData(): ChartDataPoint[] {
  ...
  function calculateVelocityData(): ChartDataPoint[] {
    const now = new Date();
    const periods = [];
  ...
  function calculateBurndownData(): ChartDataPoint[] {
    const now = new Date();
    const periods = [];
  ...
  function calculateRiskData(): ChartDataPoint[] {
  ...
  function calculateWorkloadData(): ChartDataPoint[] {
  ...
  const getBarColor = (index: number, label?: string): string => {
    if (type === 'risk') {
      switch (label) {
      ...
      <div className="space-y-4">
        {chartData.map((item, index) => (
          <div key={item.label} className="flex items-center">
            {/* Label */}
            <div className="w-24 text-sm text-gray-400 truncate" title={item.label}>
              {item.label}
            </div>
            ...
            <div className="flex-1 ml-3">
              <div className="flex items-center">
                ...
                {/* Value */}
                <div className="ml-3 text-sm text-white font-mono w-16 text-right">
      ...
      <div className="mt-6 pt-4 border-t border-gray-700">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div className="text-center">
            <div className="text-lg font-bold text-white">{trrs.length}</div>
            <div className="text-gray-400">Total TRRs</div>
          </div>
...
export default TRRProgressChart;
