...
/**
 * Firebase AI Logic Service - Migrated from henryreed.ai
 *
 ...
 * Note: Firebase app must be initialized before using AI features
 */
async function getFirebaseApp() {
  ...
  try {
    ...
    // Firebase app is typically accessed via the firebase config
    // If not exported, we'll need to initialize it separately
    return (db as any).app || null;
  } catch (error) {
 ...
 */
export async function getModel(modelName: string = DEFAULT_MODEL) {
...
export class ChatSession {
  ...
  constructor(modelName?: string) {
    this.modelPromise = getModel(modelName);
  }
